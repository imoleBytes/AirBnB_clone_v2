#!/usr/bin/python3
"""fabric module"""
from fabric.api import local, put, run, sudo, cd, lcd, env
...
def deploy():
    """Function creates and distributes an archive to your web servers"""
...
def do_clean(number=0):
    ...
    if number in [0, 1]:
        ...
        to_remove = number + 1
        with lcd("versions/"):
            local(f"ls -t versions/ | tail -n +{to_remove} | xargs rm -f")
        with cd("/data/web_static/releases/"):
            sudo(f"ls -t | tail -n +{to_remove} | xargs rm -rf")
    else:
